...
import SmartTextboxStylized from '@/components/demos/SmartTextboxStylized';
...
<Preview
  demo={<SmartTextboxStylized />}
  source={`<div className="rounded-2xl bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-500 p-[1.5px]">
  <div className="flex items-center gap-2 rounded-[14px] bg-card px-3">
    <span>✦</span>
    <SmartTextbox
      value={value}
      onChange={setValue}
      ...
      ghostStyle={{ color: '#a855f7', opacity: 0.7, fontStyle: 'italic' }}
    />
    <kbd className="rounded-md border bg-background px-1.5 py-0.5 text-[10px]">→</kbd>
  </div>
</div>`}
/>
